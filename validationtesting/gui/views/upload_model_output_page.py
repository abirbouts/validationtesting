...
def upload_model_output(resource) -> None:
    ...
    if st.session_state[f'{resource}_data_uploaded']:
        ...
        st.session_state[f'{resource}_model_output_scope'] = st.radio("Is the output defined per unit or in total?", ("Per Unit", "Total"))

        uploaded_file, delimiter, decimal = csv_upload_interface(f"power_production")
        if uploaded_file:
            ...
            with st.expander(f"Data", expanded=False):
                ...
                if st.session_state[f'{resource}_model_output_scope'] == "Per Unit":
                    for unit in range(st.session_state[f'{resource}_num_units']):
                        ...
                        data_dict[f'Model {resource} Power Unit {unit + 1}'] = power_output_data.values.flatten() if power_output_data is not None else None
                    data_dict[f'Model {resource} Power Total'] = sum(data for data in data_dict.values() if data is not None)
